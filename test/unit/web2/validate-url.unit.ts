...
import { HTTP_METHOD } from '../../../src/config/interfaces/web2-json';
...
describe('Validate-url unit tests', () => {
  ...
  it('Should reject - encoded IPv4 localhost', async () => {
    ...
    await expect(
      validateUrl(input, [], web2JsonDefaultParams.maxUrlLength),
  ...
  it('Should not reject - allowed hostname', async () => {
    ...
    const checkedUrl = await validateUrl(
      input,
      ['google.com'],
      web2JsonDefaultParams.maxUrlLength,
    );
  ...
  it('Should reject - not allowed hostname', async () => {
    const input = 'https://www.Google.com/';
    await expect(
      validateUrl(input, ['bing.com'], web2JsonDefaultParams.maxUrlLength),
