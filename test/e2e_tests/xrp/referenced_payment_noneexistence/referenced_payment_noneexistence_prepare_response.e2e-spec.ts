...
describe("/ReferencedPaymentNonexistence/prepareResponse", () => {
    it("should get abiEncodedRequest", async () => {
        ...
        expect(response.body.response.lowestUsedTimestamp).to.be.equal('1733476003');
        expect(response.body.response.requestBody.minimalBlockNumber).to.be.equal('2882022');
        expect(response.body.response.requestBody.deadlineBlockNumber).to.be.equal('2882130');
        expect(response.body.response.requestBody.deadlineTimestamp).to.be.equal('1733476340');
        expect(response.body.response.requestBody.destinationAddressHash).to.be.equal(standardAddressHash("rw33QqCywPVJRqJTmL1UYHqKPCxNC7eT6T"));
        expect(response.body.response.requestBody.amount).to.be.equal('5146300010');
        expect(response.body.response.requestBody.standardPaymentReference).to.be.equal('0x464250526641000100000000000000000000000000000000000000000006c110');
        expect(response.body.response.requestBody.checkSourceAddresses).to.be.equal(true);
        expect(response.body.response.requestBody.sourceAddressesRoot).to.be.equal("0x7006716ece630ed05048e9f87debac44c23292fbd9b022942321e9a78e4255cc");
        expect(response.body.response.responseBody.minimalBlockTimestamp).to.be.equal('2882022');
