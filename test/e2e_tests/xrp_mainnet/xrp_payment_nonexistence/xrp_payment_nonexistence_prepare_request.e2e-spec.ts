...
// Nonexistence window [103751341, 103751441]; the matching payment is at ledger 103751442
const ATTEST_TYPE =
...
describe(`/XRPPaymentNonexistence/prepareRequest (${getTestFile(__filename)})`, () => {
  baseHooks();

  it('should return VALID for a known mainnet nonexistence window', async () => {
    const response = await request(app.getHttpServer())
      .post('/XRPPaymentNonexistence/prepareRequest')
      .send({
  ...
  it('should return 400 for invalid proofOwner', async () => {
    await request(app.getHttpServer())
      .post('/XRPPaymentNonexistence/prepareRequest')
      .send({
        attestationType: ATTEST_TYPE,
        sourceId: SOURCE_ID,
        requestBody: { ...VALID_BODY, proofOwner: 'not-an-address' },
      })
      .set('X-API-KEY', '12345')
      .expect(400);
  });

  it('should return 400 for non-numeric minimalBlockNumber', async () => {
    await request(app.getHttpServer())
      .post('/XRPPaymentNonexistence/prepareRequest')
      .send({
        attestationType: ATTEST_TYPE,
        sourceId: SOURCE_ID,
        requestBody: { ...VALID_BODY, minimalBlockNumber: 'abc' },
      })
      .set('X-API-KEY', '12345')
      .expect(400);
  });
