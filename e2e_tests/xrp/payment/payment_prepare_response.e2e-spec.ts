...
describe("/Payment/prepareResponse", () => {
    it("should get abiEncodedRequest", async () => {
        ...
        expect(response.body.response.lowestUsedTimestamp).to.be.equal('2680162100');
        expect(response.body.response.requestBody.transactionId).to.be.equal('0xa7182f8cac67e7b0e16cca46e2b40abb01565d207d525a6f0a27c057f35f800c');
        ...
        expect(response.body.response.responseBody.blockNumber).to.be.equal('2882442');
        expect(response.body.response.responseBody.blockTimestamp).to.be.equal('1733477300');
        expect(response.body.response.responseBody.sourceAddressHash).to.be.equal(standardAddressHash("rnX8aKTwshNqCHQX9AwG5YVC8N7ADnvDaJ"));
        expect(response.body.response.responseBody.sourceAddressesRoot).to.be.equal('0x4fe91421576e4f6e2799d2db04fbf187e856a29bada4de0b10ca4f4591a4d3d0');
        expect(response.body.response.responseBody.receivingAddressHash).to.be.equal(standardAddressHash("rPkCVtwNcAuxmMRhDSTs3VYz9pwEm6TExy"));
        expect(response.body.response.responseBody.intendedReceivingAddressHash).to.be.equal(standardAddressHash("rPkCVtwNcAuxmMRhDSTs3VYz9pwEm6TExy"));
        expect(response.body.response.responseBody.standardPaymentReference).to.be.equal('0x46425052664100020000000000000000000000000000000000000000050eab96');
        expect(response.body.response.responseBody.spentAmount).to.be.equal('39960012');
        expect(response.body.response.responseBody.intendedSpentAmount).to.be.equal('39960012');
        expect(response.body.response.responseBody.receivedAmount).to.be.equal('39960000');
        expect(response.body.response.responseBody.intendedReceivedAmount).to.be.equal('39960000');
    ...
    it("should get bad request (400) for too long transactionId", async () => {
        const payload = {
            ...
            sourceId: "0x7465737458525000000000000000000000000000000000000000000000000000",
            requestBody: {
                transactionId: "a7182f8cac67e7b0e16cca46e2b40abb01565d207d525a6f0a27c057f35f800c0",
                inUtxo: "0",
                utxo: "0"
            }
        }
