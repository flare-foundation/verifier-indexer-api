...
describe("/BalanceDecreasingTransaction/prepareResponse", () => {
    it("should get valid status", async () => {
        ...
        expect(res.sourceId).to.be.equal('0x7465737458525000000000000000000000000000000000000000000000000000');
        expect(res.votingRound).to.be.equal('0');
        expect(res.lowestUsedTimestamp).to.be.equal('2680162100');
        expect(res.requestBody.transactionId).to.be.equal('0xa7182f8cac67e7b0e16cca46e2b40abb01565d207d525a6f0a27c057f35f800c');
        expect(res.requestBody.sourceAddressIndicator).to.be.equal(standardAddressHash("rnX8aKTwshNqCHQX9AwG5YVC8N7ADnvDaJ"));
        expect(res.responseBody.blockNumber).to.be.equal("2882442");
        expect(res.responseBody.blockTimestamp).to.be.equal("1733477300");
        expect(res.responseBody.sourceAddressHash).to.be.equal("0x87a56dd79e047e8ec5523d8c7df42cd304b7070656b93554f2a20e055690dfaf");
        expect(res.responseBody.spentAmount).to.be.equal("39960012");
        expect(res.responseBody.standardPaymentReference).to.be.equal("0x46425052664100020000000000000000000000000000000000000000050eab96");
    });
