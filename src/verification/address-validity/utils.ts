import { standardAddressHash, ZERO_BYTES_32 } from '@flarenetwork/mcc';
...
export function base58Checksum(decodedAddress: Buffer): boolean {
  ...
  const hash1 = createHash('sha256')
    .update(decodedAddress.subarray(0, -4))
    .digest();
...
export function validAddressToResponse(
  address: string,
  lowercase: boolean,
  ...
  return {
    ...
    standardAddressHash: standardAddressHash(standardAddress),
  };
}
