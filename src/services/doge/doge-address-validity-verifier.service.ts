...
export class DOGEAddressValidityVerifierService {
  ...
  constructor() {
    ...
    // this.exampleData = JSON.parse(
    //   readFileSync(
    //     'src/config/exaple-openapi-data/doge/AddressValidity.json',
    //     'utf8',
    //   ),
    // );
  }
  ...
  async verifyRequestInternal(
    request: AddressValidity_Request | AddressValidity_RequestNoMic,
  ...
  async verifyRequest(
    ...
    const result = verifyAddressDOGE(fixedRequest.requestBody.addressStr);
  ...
  public async verifyEncodedRequest(
    ...
    const requestJSON =
      this.store.parseRequest<AddressValidity_Request>(abiEncodedRequest);
    const response = await this.verifyRequestInternal(requestJSON);
    return response;
  }

  public async prepareResponse(
    request: AddressValidity_RequestNoMic,
  ): Promise<AttestationResponseDTO_AddressValidity_Response> {
